...
class CrossAttentionTransformerBlock(nn.Module):
    """
    A Transformer block that performs:
      1) Self-attention on x
      2) Cross-attention on c
      3) Feed-forward (conv-based)
      4) Conditional LN on diffusion time
    """
    def __init__(self, embed_dim, num_heads, ff_dim, conditioning_dim, dropout=0.1):
        ...
        # For feed-forward, you can still use your ConvolutionalFeedForward
    ...
    def forward(self, x, c, time_emb, x_mask=None, c_mask=None):
        """
        x: (B, Nx, E) main stream (prosody tokens)
        c: (B, Nc, E) conditioning stream (e.g. c_ph)
        time_emb: (B, E) diffusion time embedding
        """
        # 1) Self-attn
        attn_out, _ = self.self_attn(x, x, x, attn_mask=x_mask, need_weights=False)
        x = x + attn_out
        x = self.cond_ln1(x, time_emb)

        # 2) Cross-attn
        cross_out, _ = self.cross_attn(x, c, c, attn_mask=c_mask, need_weights=False)
        x = x + cross_out
...
class ProsodyDiffusionModel(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.prosody_embedding = nn.Embedding(prosody_vocab_size, embed_dim)
        self.time_embed = TimeEmbedding(embed_dim)
    ...
    def forward(self, prosody_tokens, c_ph, t, x_mask=None, c_mask=None):
        """
        prosody_tokens: (B, Nx) => partial-noise prosody tokens [prompt + masked target]
        c_ph: (B, Nc, E) => e.g. frame-level phoneme condition
        t: (B,) => diffusion time steps
        x_mask, c_mask: optional attn masks
        """
        # (B, Nx, E)
        x = self.prosody_embedding(prosody_tokens)
        time_emb = self.time_embed(t)  # (B, E)

        # pass through 12 cross-attn blocks
        for layer in self.layers:
            x = layer(x, c_ph, time_emb, x_mask=x_mask, c_mask=c_mask)

        logits = self.output_linear(x)  # (B, Nx, prosody_vocab_size)
        return x, logits
